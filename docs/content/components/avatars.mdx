...
[`ProfilePreview`](/components/profile-preview)s in a grid, e.g., authors of a
blog posts or speakers at an event. It is an alternative to using
...
`Avatars` supports [space props](https://styled-system.com/api#space) and
...
| align  | `start`, `center` or `end` | `start` |          | Alignment of profile (responsive). |
...
- [Few avatars (themed with `@undataforum/preset`)](/avatars/few-avatars)
- [Many avatars (themed with `@undataforum/preset`)](/avatars/many-avatars)
